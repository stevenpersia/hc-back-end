...
function setFilters(req) {
	...
	if (req.query.category) {
		...
		for (let i = 0; i < tab.length; i++) {
			if (tab[i] === 'environnement') cat = '5c0560a6c1215431d47a98e5';
			if (tab[i] === 'animaux') {
...
router.get('/', function(req, res) {
	...
	let distance = 50000;
	// centre de la recherche par défault LION
	let lat = 48.8523281;
	let long = 2.3720629;
	...
	Challenge.find(filters)
		...
		.populate('ref.category owner challengers')
...
router.get('/:id', function(req, res) {
	Challenge.findById(req.params.id)
		.populate('ref.category owner challengers badge ref.prerequisites')
